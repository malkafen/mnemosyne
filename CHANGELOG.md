...
- **VMs created by 0.2.0 block the run.** They carry no `<mnem:init>`, and a managed VM of the
  inventory without one is `blocked` rather than guessed about. Add the marker to each of them,
  inside the `<mnem:mnemosyne>` element, before the first 0.3.0 run —
  `virsh metadata <domain> https://mnemosyne.dev/schema/v1 --key mnem --config --edit`
  (with `--live` as well for a running domain) and add `<mnem:init state="adopted"/>` — or
  recreate them.
- **`launch` is enforced on every run.** A managed VM that is running while the inventory says
  `launch: false` is shut down, and one that is off while it says `launch: true` is started.
  Check the plan first.
- **Exit codes are stricter.** A run with a skipped entry, a guest that never phoned home or a VM
  still `pending` now exits `1`, where 0.2.0 exited `0`. Schedulers and CI jobs that treated `0`
  as "applied" now see the difference.
- **Reused volumes are no longer deleted with their VM.** Volumes 0.2.0 recorded in
  `<mnem:disks>` are still treated as Mnemosyne's own.
...
- Disks are reconciled on every run, add-only: one the inventory lists and the domain lacks is
  ...
  Nothing is ever detached, shrunk or deleted while the VM exists: a disk missing from the
  inventory and a changed `pool` are reported as `note` lines and left alone, and a size is
  grown or refused as described under Added. A volume that already exists under the expected
  name is reused rather than replaced, reported as `reused existing volume`. Deleting the VM
  deletes the volumes Mnemosyne created for it, and the plan names every one of them first.
...
- The README is a short overview with a quick start; the details moved to `docs/` — how a run
  works, cloud-init, configuration, disks and output — with mermaid diagrams of the run, the
  seed exchange and the init marker.
...
- An inventory where two groups target the same `host:port` is rejected before anything is
  contacted. Deletion is scoped to the host, not the group, so each group would have deleted the
  other's VMs as absent from the inventory.
